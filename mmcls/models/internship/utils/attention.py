...
class MultiheadAttentionQ(BaseModule):
    ...
    def __init__(self,
                 ...
                 v_shortcut=False,
                 init_cfg=None):
        super(MultiheadAttentionQ, self).__init__(init_cfg=init_cfg)
        self.quant = torch.quantization.QuantStub()
        self.dequant = torch.quantization.DeQuantStub()
        self.f_mul = torch.nn.quantized.FloatFunctional()
        self.f_add = torch.nn.quantized.FloatFunctional()
    ...
    def forward(self, x):
        ...
        x = self.matmul.matmul(attn, v).transpose(1, 2).reshape(B, N, self.embed_dims)
