...
class SwinTransformerQ(BaseBackbone):
    """Swin Transformer.
    ...
    def __init__(self,
        ...
        for i, (depth,
            ...
            # self.stage_quant.append(torch.quantization.QuantStub())
    ...
    def init_weights(self):
        super(SwinTransformerQ, self).init_weights()
    ...
    def forward(self, x):
        ...
        for i, stage in enumerate(self.stages):
            ...
            if i in self.out_indices:
                ...
                outs.append(out)
            x = self.dequant(x) # needed because of next stage
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, *args,
        ...
        if (version is None
                or version < 2) and self.__class__ is SwinTransformerQ:
            ...
            for k in state_dict_keys:
                if k.startswith('norm.') or k.startswith('backbone.norm.'):
                    ...
                    del state_dict[k]
        if (version is None
                or version < 3) and self.__class__ is SwinTransformerQ:
            state_dict_keys = list(state_dict.keys())
            for k in state_dict_keys:
                if 'attn_mask' in k:
                    del state_dict[k]
    ...
    def train(self, mode=True):
        super(SwinTransformerQ, self).train(mode)
        self._freeze_stages()
